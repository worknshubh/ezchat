...
class chatroom : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        user_message_send_btn.setOnClickListener{
            ...
            db.collection("chatroom").document(chatroomid).collection("messages").add(messageData).addOnSuccessListener {
                Toast.makeText(this,"Message Sent Successfully",Toast.LENGTH_LONG).show()
                user_message.text.clear()
            }
                .addOnFailureListener{
                    Toast.makeText(this,"Unable to Send the Mesasage Right Now ",Toast.LENGTH_LONG).show()
                }
        }
    }
